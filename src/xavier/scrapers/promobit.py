...
class PromobitScraper(Scraper):
    name = "promobit"
    base_url = "https://www.promobit.com.br"
    listing_path = "/cupom-desconto/mercado-livre/"
    priority = 3
    ...
    def _parse(self, soup: BeautifulSoup, page_url: str) -> Iterable[Coupon]:
        # Promobit usa cards de promoção/cupom; tentamos várias variações.
        cards = soup.select(
            "article, [data-testid*='coupon'], [data-testid*='promo'], "
            ".thread-item, .coupon-card"
        )
        seen: set[str] = set()
        for card in cards:
            ...
            yield Coupon(
                code=code,
                description=self._first_text(card, ["h2", "h3", ".title", "[data-testid*='title']"]),
    ...
    def _extract_code(card: Tag) -> str | None:
        for attr in ("data-coupon", "data-clipboard-text", "data-code"):
            v = card.get(attr)
